...
- (void) bid {
    //BID START
    if (self.playerBidLabel.hidden != YES || self.playerTricksTakenLabel.hidden != YES || self.playerWhoWonTrickLabel.hidden != YES || self.suitLabel.hidden != YES || self.valueLabel.hidden != YES || self.suitInputTextField.hidden != YES || self.valueInputTextField.hidden != YES || self.suitInputTextField.enabled != NO || self.valueInputTextField.enabled != NO || self.promptLabel.hidden != YES || self.playButton.hidden != YES || self.currentTrickLabel.text != @"" || self.currentTrickTabelView.hidden != YES || biddingPromptLabel.hidden != NO || biddingInputTextField.hidden != NO || bidButton.hidden != NO || biddingInputTextField.enabled != YES) {
    ...
    if ([self.bidArray count] < self.numberOfPlayers) {
        ...
        //BID END
        if (biddingPromptLabel.hidden != YES || biddingInputTextField.hidden != YES || bidButton.hidden != YES || self.playerBidLabel.hidden != NO || self.playerTricksTakenLabel.hidden != NO || self.playerWhoWonTrickLabel.hidden != NO || self.suitLabel.hidden != NO || self.valueLabel.hidden != NO || self.suitInputTextField.hidden != NO || self.valueInputTextField.hidden != NO || self.suitInputTextField.enabled != YES || self.valueInputTextField.enabled != YES || self.promptLabel.hidden != NO || self.playButton.hidden != NO || self.currentTrickLabel.text != @"Current Trick:" || self.currentTrickTabelView.hidden != NO || biddingPromptLabel.hidden != YES || biddingInputTextField.hidden != YES || bidButton.hidden != YES || biddingInputTextField.enabled != NO) {
...
- (void) play {
    if ([[self.playersArray objectAtIndex:self.playerWhoseTurnItIs%self.numberOfPlayers] typeOfPlayer] == PlayerTypeHuman) {
        ...
        [self.currentTrick addObject:[[[[self.playersArray objectAtIndex:playerIndex] currentHand] filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"suit == %d AND value == %d", [self.suitInputTextField.text intValue], [self.valueInputTextField.text intValue]]] objectAtIndex:0]];
        
        //removes the currently played card from that player's hand
        [[[self.playersArray objectAtIndex:playerIndex] currentHand] removeObject:[[[[self.playersArray objectAtIndex:self.playerWhoseTurnItIs%self.numberOfPlayers] currentHand] filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"suit == %d AND value == %d", [self.suitInputTextField.text intValue], [self.valueInputTextField.text intValue]]] objectAtIndex:0]];
